...
const u32 RowArray[5] = {BIT9,BIT0,BIT8,BIT2,BIT3};
//const u32 RowArray[5] = {BIT0,BIT2,BIT3};

const u32 LineArray[5] = {BIT10,BIT7,BIT5,BIT4,BIT1};
...
{
    //RowArray
    sys_set_port_mux(PAD_GPIO_09, (PAD_MUX_FUNCTION_0 | 0x02));
    sys_set_port_mux(PAD_GPIO_00, (PAD_MUX_FUNCTION_0 | 0x02));
    sys_set_port_mux(PAD_GPIO_08, (PAD_MUX_FUNCTION_0 | 0x02));
    sys_set_port_mux(PAD_GPIO_02, (PAD_MUX_FUNCTION_0 | 0x02));
   ...
   //LineArray
    ...
    memset(&kb_data,0,sizeof(kb_data_t));
    ty_record_time = hal_clock_get_system_tick()/HAL_CLOCK_1MS_TICKS;
}
...
void app_matrix_key_temp_clear(void)
{
    key_press_flag = 0;
    memset(&kb_data_record,0,sizeof(kb_data_t));
...
void app_matrix_key_run(void)
{
    ...
    if((key_state != NONE_PRESS)&&(key_state < LONG_PRESS_START)&&((now_time) > KEY_LONG_PRESS_TIME + ty_record_time)){
