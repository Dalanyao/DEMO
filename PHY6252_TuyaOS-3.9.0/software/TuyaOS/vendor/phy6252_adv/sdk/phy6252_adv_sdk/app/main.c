...
extern VOID_T tal_framework_init_first(VOID_T);
extern VOID_T tal_framework_init_second(VOID_T);
extern VOID_T tal_framework_init_third(VOID_T);
extern VOID_T tal_framework_init_last(VOID_T);
extern VOID_T tal_framework_loop(VOID_T);
...
static void hal_rfphy_init(void)
{
    ...
    g_rfPhyFreqOffSet   =RF_PHY_FREQ_FOFF_N60KHZ; //RF_PHY_FREQ_FOFF_N120KHZ
    //============config xtal 16M cap
    XTAL16M_CAP_SETTING(0x00);
...
static void hal_init(void)
{
    ...
    tal_framework_init_first();
    tal_framework_init_second();
    hal_gpio_init();
    const uint8_t crc_check[4] = {0x5A, 0x5A, 0xA5, 0xA5};
    uint8_t crc_check_read[4] = {0};
    
    hal_flash_read(0x11003ff0, crc_check_read, 4);
    if(0 != memcmp(crc_check, crc_check_read, 4)){
        hal_flash_write(0x11003ff0, (uint8_t*)crc_check, 4);
    }
	
	hal_storage_256_init();
	
    tal_framework_init_third();
}
...
int  main(void)
{
    g_system_clk = SYS_CLK_DLL_48M;//SYS_CLK_DLL_64M;//SYS_CLK_DLL_48M;//SYS_CLK_DLL_64M;
    ...
    TY_PRINTF("SDK Version ID %08x \n",SDK_VER_RELEASE_ID);
    TY_PRINTF("rfClk %d rcClk %d sysClk %d tpCap[%02x %02x]\n",g_rfPhyClkSel,g_clk32K_config,g_system_clk,g_rfPhyTpCal0,g_rfPhyTpCal1);
    app_main();
}
